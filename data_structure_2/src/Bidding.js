...
Bidding.get_sort = function (list_price_and_number, activity_id, bid_id) {
    ...
    var price = _.find(list_price_and_number,function (list) {
    ...
    return [
        {name: sign_up.name, phone: sign_up.phone, price: winner[0]["price"]}
    ]
}
...
Bidding.bid_is_or_not_sign_up = function (phone) {
    var activities = Activity.activities();
    var activity = Activity.current_activity(activities);
    return _.some(activity["sign_ups"], function (list) {
...
Bidding.is_or_no_bid_repeat = function (json_message) {
    ...
    return _.some(bidder, function (list) {
        return list["phone"] == JJ_phone
    })
}
...
function transform_biddings_to_view_model(activity_id, bid_id) {
    var bid_message = Bidding.get_bid_message(activity_id, bid_id)
...
function is_or_not_bid_success(list_price_and_number, activity_id, bid_id) {
