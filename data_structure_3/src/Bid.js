...
Bid.create_new_bid_activity_name=function(activity_id){
    ...
    var activity= _.find(all_activity,function(list){return list.activity_id==activity_id})
...
Bid.render_bids=function(activity_id){
   ...
   var b= _.groupBy(bid_for_activity,function(list){return list.name})
   var list_bid_name = [];
   _.map(b, function (value, key) {list_bid_name.push({"name": key, "number": value.length}) })
